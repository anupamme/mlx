...
#include "mlx/backend/cpu/task.h"
...
array eval_impl(std::vector<array> outputs, bool async) {
  ...
  while (!tape.empty()) {
    ...
    bool signal = typeid(arr.primitive()) == typeid(Synchronizer);

    if (arr.primitive().device() == Device::gpu) {
      metal::eval(std::move(arr), signal);
    } else {
      cpu::eval(std::move(arr), signal);
    }
    if (needs_fence.find(arr.id()) != needs_fence.end()) {
      // TODO add siblings here
      fences[stream.index].update(stream, arr);
    }
  }
